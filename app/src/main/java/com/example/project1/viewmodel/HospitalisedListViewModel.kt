...
class HospitalisedListViewModel: ViewModel() {
    ...
    fun updatePatient(patient: Patient){
        for(p in patientsList){
            if(p.id == patient.id){
                ...
                p.comingState = patient.comingState
                p.currentState = patient.currentState
                break
            }
        }

        val listToSubmit = mutableListOf<Patient>()
        listToSubmit.addAll(getPatientsList(patient.hospital))
        patients.value = listToSubmit
